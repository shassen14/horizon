...
class FeatureEngine:
    ...
    async def _write_features_to_db(self, df: pl.DataFrame):
        ...
        # Get a list of all columns in the FeaturesDaily model (excluding PKs for update)
        # This is a robust way to ensure we only try to insert what exists
        ...
        async with get_db_session() as session:
            # We use an "upsert" statement.
            # If a row for (time, asset_id) exists, it gets UPDATED.
            # If not, it gets INSERTED.
            ...
            # Chunking isn't strictly necessary here because we process one stock at a time
            # (max ~5000 rows), but it's good practice.
            ...
            for i in range(0, total_records, chunk_size):
                ...
                await session.execute(
                    insert(FeaturesDaily)
                    .values(chunk)
                    .on_conflict_do_update(
                        index_elements=["time", "asset_id"], set_=update_dict
                    )
                )
